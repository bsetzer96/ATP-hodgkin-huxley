...
#    PY2.Iapp = '5 *amp*(10**-6) * cm ** -2'
 ...
 #'''_post=0 : amp * meter ** -2 (summed)'''   
    ...
    S_FSPY=Synapses(FS,PY,model='IsynFS'+eq_syn,method='rk4')
    S_FSPY.connect(i=0, j=[0,1]) #i=0, j=0
    S_FSPY.g_i=(0.3) * msiemens * cm **-2 #0.64
    S_FSPY.tau_i=5*ms
    S_FSPY.E_i= -80*mV
    S_FSPY.b_i=2
    S_FSPY.a_i= 1 #4
    ...
    INa2=StateMonitor(FS,'INa',record=[0])
    IK2=StateMonitor(FS,'IK',record=[0])
    Iran2=StateMonitor(FS, 'Iran', record=[0])
    
    GABA=StateMonitor(S_FSPY, 'IsynFS', record=True)
    
    all_neurons= PY, FS
    all_synapses= S_PYPY, S_FSPY
    all_monitors= V1, ATP,Na, INa, IK, S, AMPA, V2, INa2, IK2, Iran2, GABA
    #am=AMPA
...
if __name__=='__main__':
    ...
    figure()
    V2=all_monitors[7]
    plot(V2.t/second,V2.V[0]/mV)
    xlabel('Time (s)')
    ylabel('Membrane potential (mV)')
    title('FS cell')
    figure()
    gb=all_monitors[11]
    plot(gb.t/second,gb.IsynFS[0]/second)
    xlabel('Time (s)')
    ylabel('')
    title('GABA')
    clear_cache('cython')
